...
// Fetch ONE card by Question ID with a server-side filterByFormula, instead of
// pulling the whole deck and searching it. Used by the per-card metadata layout
// and the /card/<qid>/image proxy, where one request = one card.
// Applies the same public gate as getQuestions(): published and not held.
// Returns null when the card doesn't exist or isn't public.
export async function getQuestionById(qid, { includeDrafts = false, includeHeld = false } = {}) {
  const id = String(qid || '').trim();
  if (!id) return null;
  const recs = await listRecords(QUESTIONS_TABLE, {
    filterByFormula: `{Question ID} = ${formulaString(id)}`,
    maxRecords: 1,
  });
  if (!recs.length) return null;
  const q = shape(recs[0]);
  if (!includeHeld && q.needsReview) return null;
  if (!includeDrafts && !q.published) return null;
  return q;
}
